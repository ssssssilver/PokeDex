...
function resolveAssets(value) {
  ...
  if (value.indexOf('__OSS_BASE__') === 0)
    return `${baseUrl()}${value.slice('__OSS_BASE__'.length)}`
...
function requestJson(relativePath, options) {
  ...
  const promise = new Promise((resolve, reject) => {
    if (!Taro || !Taro.request) {
      reject(new Error('Taro.request is unavailable'))
      return
    }
    Taro.request({
      url,
  ...
  if (useCache)
    responseCache[url] = promise.catch((error) => {
...
function getManifest() {
  if (!manifestPromise) {
    manifestPromise = requestJson(`manifest.json?v=${Date.now()}`, {
      cache: false,
      fresh: true,
    })
      .then((manifest) => {
        if (Number(manifest.schemaVersion) !== 1)
          throw new Error('Unsupported OSS schema')
        return manifest
      })
      .catch((error) => {
...
function getListChunk(manifest, name, index) {
  ...
  return requestJson(releasePath(manifest, path)).then(
    (result) => result.items || []
  )
}
function getAllRows(manifest, name) {
  ...
  if (!datasetCache[key]) {
    ...
    datasetCache[key] = Promise.all(
      Array.from(
        {
          length: count,
        },
        (unused, index) => getListChunk(manifest, name, index)
      )
...
function getDefaultPage(manifest, name, page, pageSize) {
  ...
  for (let index = firstChunk; index <= lastChunk; index += 1)
    requests.push(getListChunk(manifest, name, index))
...
function defaultPage(manifest, name, options) {
  ...
  return getDefaultPage(manifest, name, page, pageSize).then((items) => ({
    ...
    totalPages: Math.max(
      1,
      Math.ceil(manifest.datasets[name].count / pageSize)
    ),
...
function listPokemon(manifest, options) {
  ...
  const keyword = String(filters.keyword || '')
    .trim()
    .toLowerCase()
  const types = String(filters.type || filters.types || '')
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean)
  ...
  return getAllRows(manifest, 'pokemon').then((rows) => {
    ...
    const items = rows.filter((item) => {
      if (
        keyword &&
        ![item.id, item.name_en, item.name_zh, item.name_ja]
          .join(' ')
          .toLowerCase()
          .includes(keyword)
      )
      ...
      if (
        pokedex !== 'national' &&
        !keys.some((key) => (item.regional_dex_keys || []).includes(key))
      )
        return false
      return true
    })
    if (sort === 'name')
      items.sort((a, b) =>
        String(a.name_zh || a.name_en).localeCompare(
          String(b.name_zh || b.name_en),
          'zh'
        )
      )
    if (sort === 'power')
      items.sort(
        (a, b) => Number(b.stat_total || 0) - Number(a.stat_total || 0)
      )
...
function listCards(manifest, options) {
  const filters = options || {}
  const keyword = String(filters.keyword || filters.q || '')
    .trim()
    .toLowerCase()
  const type = String(filters.type || '').trim()
  const setId = String(filters.setId || '')
    .trim()
    .toLowerCase()
  const supertype = String(filters.supertype || '')
    .trim()
    .toLowerCase()
  const rarity = String(filters.rarity || '')
    .trim()
    .toLowerCase()
  ...
  return getAllRows(manifest, 'ptcg').then((rows) => {
    const items = rows.filter((item) => {
      if (
        ...
        ![
          ...
          .concat(item.type_names || [], item.subtype_names || [])
          .join(' ')
          .toLowerCase()
          .includes(keyword)
      )
      ...
      if (setId && String(item.set_id || '').toLowerCase() !== setId)
        return false
      if (supertype && String(item.supertype || '').toLowerCase() !== supertype)
        return false
      if (rarity && String(item.rarity || '').toLowerCase() !== rarity)
        return false
      if (
        pokemonId &&
        !(item.national_pokedex_numbers || []).map(Number).includes(pokemonId)
      )
        return false
      return true
    })
    ...
    if (sort === 'rarity')
      items.sort((a, b) =>
        String(a.rarity_name || a.rarity).localeCompare(
          String(b.rarity_name || b.rarity),
          'zh'
        )
      )
    return paginated(items, filters)
...
function listPocketCards(manifest, options) {
  ...
  const keyword = String(filters.keyword || filters.q || '')
    .trim()
    .toLowerCase()
  const expansion = String(filters.expansion || '')
    .trim()
    .toUpperCase()
  const rarity = String(filters.rarity || '')
    .trim()
  ...
  return getAllRows(manifest, 'pocket').then((rows) => {
    const items = rows.filter((item) => {
      ...
      if (
        keyword &&
        ![item.id, item.name_zh, item.name_en, item.artist, item.pack_name_en]
          .concat(
            collections.map(
              (entry) =>
                `${entry.expansion_id} ${entry.expansion_name_zh} ${entry.number}`
            )
          )
          .join(' ')
          .toLowerCase()
      ...
      if (
        expansion &&
        !collections.some(
          (entry) => String(entry.expansion_id).toUpperCase() === expansion
        )
      )
        return false
      if (rarity && String(item.rarity || '').toUpperCase() !== rarity)
        return false
      if (type && String(item.card_type || '').toLowerCase() !== type)
        return false
      if (pokemonId && Number(item.national_pokedex_number || 0) !== pokemonId)
        return false
      return true
    })
    if (sort === 'name')
...
function dailyQuiz(manifest, name, seed) {
  ...
  return Promise.all(
    Object.keys(byChunk).map((chunkIndex) =>
      getListChunk(manifest, name, Number(chunkIndex)).then((rows) => ({
    ...
    chunks.forEach((entry) => {
      byChunk[entry.chunkIndex].forEach((absoluteIndex) => {
        lookup[absoluteIndex] =
          entry.rows[absoluteIndex - entry.chunkIndex * dataset.listChunkSize]
      })
    })
    ...
    if (name === 'pokemon') {
      return {
        item: {
          ...
          hints: [
            `属性：${(answer.typeNames || answer.types || []).join(' / ')}`,
    ...
    return {
      item: {
        ...
        hints: [
          `Set: ${answer.set_name}`,
          `Type: ${(answer.type_names || []).join(' / ')}`,
          `Rarity: ${answer.rarity_name || answer.rarity}`,
          `No: ${answer.set_id} #${answer.number}`,
        ],
        options: choices.map((item) => ({
          id: item.id,
          name: item.name,
...
function weightedKey(weights) {
  ...
  let target =
    Math.random() * entries.reduce((sum, entry) => sum + Number(entry[1]), 0)
...
function openCardPack(manifest, payload) {
  return getAllRows(manifest, 'ptcg').then((rows) => {
    const candidates = rows.filter(
      (item) => !payload.setId || String(item.set_id) === String(payload.setId)
    )
...
function openPocketPack(manifest, payload) {
  return Promise.all([
    getAllRows(manifest, 'pocket'),
    getFile(manifest, 'pocketPacks'),
    getFile(manifest, 'pocketPullRates'),
    getFile(manifest, 'pocketRarities'),
  ]).then(([cards, packResult, ratesResult, rarityResult]) => {
    const packs = packResult.items || []
    const pack =
      packs.find(
        (item) => String(item.id) === String(payload.packId || payload.id || '')
      ) ||
      packs.find((item) => item.is_regular) ||
      packs[0]
    if (!pack)
      return {
        item: null,
    ...
    const candidates = cards.filter((card) =>
      allowed.size
        ...
        : (card.collections || []).some(
            (item) => item.expansion_id === pack.expansion_id
          )
    )
    if (pack.is_promo) {
    ...
    const pulled = Array.from(
      ...
      (unused, index) => {
        ...
        const pool = rarity
          ? candidates.filter((card) => card.rarity === rarity)
          : candidates
    ...
    return {
      item: {
        ...
        pack,
        pack_type: packType,
        is_rare_pack: /rare/i.test(packType),
        cards: pulled,
        count: pulled.length,
      },
      source: 'oss-static',
...
function analyzeTeam(manifest, payload) {
  return Promise.all([
    getAllRows(manifest, 'pokemon'),
    getFile(manifest, 'typeRelations'),
    getFile(manifest, 'types'),
  ]).then(([rows, relations, typeResult]) => {
    const members = (payload.ids || [])
      .map((id) => rows.find((item) => Number(item.id) === Number(id)))
      .filter(Boolean)
    const weaknesses = []
    const resistances = []
    const immunities = []
    ...
    const primaryTypes = Object.keys(typeCounts)
      ...
      .slice(0, 3)
      .map((type) => names[type] || type)
    const leadingWeaknesses = weaknesses
      .slice(0, 3)
      .map((item) => item.name)
      .join('、')
    const summary = members.length
      ? `当前队伍以${primaryTypes.join('、') || '混合'}属性为主，主要风险来自${
          leadingWeaknesses || '暂无明显共同弱点'
        }。`
...
function callWithManifest(manifest, action, data) {
  ...
  if (action === 'getPocketCard')
    return getDetail(manifest, 'pocket', payload.id)
  if (action === 'getTypes') return getFile(manifest, 'types')
  if (action === 'getTypeRelations')
    return getFile(manifest, 'typeRelations').then(
      (items) =>
        items[payload.type] || {
          item: null,
          source: 'oss-static',
  ...
  if (action === 'getEvolutionChain')
    return getAllRows(manifest, 'pokemon').then((items) => ({
      items: items.filter((item) =>
        (payload.ids || []).map(Number).includes(Number(item.id))
      ),
      source: 'oss-static',
    }))
  if (action === 'getDailyQuiz')
    return dailyQuiz(manifest, 'pokemon', payload.seed)
  if (action === 'submitDailyQuiz' || action === 'submitDailyCardQuiz')
  ...
  if (action === 'getSyncStatus')
    return getFile(manifest, 'pokemonStatus').then((result) =>
      Object.assign({}, result, {
        item: Object.assign({}, result.item, {
          mode: 'oss-static',
          label: 'OSS static snapshot',
          cacheReady: true,
        }),
        source: 'oss-static',
      })
  ...
  if (action === 'getPtcgSyncStatus')
    return getFile(manifest, 'ptcgStatus').then((result) =>
      Object.assign({}, result, {
        item: Object.assign({}, result.item, {
          mode: 'oss-static',
          label: 'OSS static snapshot',
  ...
  if (action === 'listPocketExpansions')
    return listMetaCollection(manifest, 'pocketExpansions', payload)
  if (action === 'listPocketPacks')
    return getFile(manifest, 'pocketPacks').then((result) =>
      paginated(
        (result.items || []).filter(
          (item) =>
            !payload.expansion ||
            String(item.expansion_id).toUpperCase() ===
              String(payload.expansion).toUpperCase()
        ),
        payload
      )
    )
  ...
  if (action === 'getPocketPullRates')
    return getFile(manifest, 'pocketPullRates').then((result) => {
      if (!payload.expansion) return result
      return {
        item: (result.item || {})[payload.expansion] || null,
        expansion: payload.expansion,
        source: 'oss-static',
      }
    })
  if (action === 'listPocketEvents')
    return listMetaCollection(manifest, 'pocketEvents', payload)
  if (action === 'listPocketMissions')
    return listMetaCollection(
      manifest,
      'pocketCollections',
      payload,
      'missions'
  ...
  if (action === 'listPocketBattles')
    return listMetaCollection(manifest, 'pocketCollections', payload, 'battles')
  if (action === 'listPocketShops')
    return listMetaCollection(manifest, 'pocketCollections', payload, 'shops')
  ...
  if (action === 'getHotDecks')
    return getFile(manifest, 'ptcgDecks').then((result) =>
      Object.assign({}, result, {
        items: (result.items || []).slice(0, Number(payload.limit || 8)),
        source: 'oss-static',
      })
    )
  if (action === 'getHotDeckDetail')
    return getFile(manifest, 'ptcgDeckDetails').then(
      (result) =>
        result.items[payload.url] || {
          item: null,
          source: 'oss-static',
        }
    )
  if (action === 'getPocketHotDeck')
    return getFile(manifest, 'pocketDeckDetails').then(
      (result) =>
  ...
  if (action === 'getSyncRuns' || action === 'getPtcgSyncRuns')
    return Promise.resolve({
      items: [],
      total: 0,
      source: 'oss-static',
  ...
  if (action === 'validateCache')
    return Promise.resolve({
      ...
      checks: {
        remoteReady: true,
        actualCount: manifest.datasets.pokemon.count,
        countOk:
          manifest.datasets.pokemon.count >=
          Number(payload.expectedCount || 1025),
      },
      status: {
        mode: 'oss-static',
        cacheReady: true,
...
function call(action, data) {
  if (!config.useStaticApi || !config.staticBaseUrl)
    return Promise.reject(new Error('OSS static API is disabled'))
  return getManifest().then((manifest) =>
    callWithManifest(manifest, action, data)
  )
}
module.exports = {
  call,
