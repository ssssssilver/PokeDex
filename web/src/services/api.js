...
const local = require('../utils/pokemon.js')
const ptcgSample = require('../data/ptcg-sample.js')
const pocketSample = require('../data/pocket-sample.js')
function hasTaro() {
  return Boolean(Taro)
}
...
function localSyncStatus() {
  ...
  return {
    item: {
      mode: config.useRemoteApi
        ? 'remote-fallback'
        : config.useCloudApi
        ? 'cloud-fallback'
        : mode,
      label:
        config.useRemoteApi || config.useCloudApi ? `${label}（兜底）` : label,
      healthLabel: '本地',
      healthTone: 'local',
      cacheReady: false,
...
function remoteRoute(action, data) {
  ...
  if (action === 'listPokemon') {
    return {
      method: 'GET',
      path: `/api/pokemon?${buildQuery(payload)}`,
    }
  }
  if (action === 'getPokemon') {
    return {
      method: 'GET',
      path: `/api/pokemon/${encodeURIComponent(payload.id)}`,
    }
  }
  if (action === 'getTypes') {
    return {
  ...
  if (action === 'getTypeRelations') {
    return {
      method: 'GET',
      path: `/api/types/${encodeURIComponent(
        payload.type || 'fire'
      )}/relations`,
    }
  }
  ...
  if (action === 'getDailyQuiz') {
    return {
      method: 'GET',
      path: `/api/quiz/daily?${buildQuery({
        seed: payload.seed,
      })}`,
    }
  }
  ...
  if (action === 'getSyncStatus') {
    return {
      method: 'GET',
      path: `/api/sync/status?${buildQuery(payload)}`,
    }
  }
  if (action === 'getSyncRuns') {
    return {
      method: 'GET',
      path: `/api/sync/runs?${buildQuery(payload)}`,
    }
  }
  if (action === 'validateCache') {
    return {
      method: 'POST',
      path: '/api/cache/validate',
      data: payload,
    }
  }
  if (action === 'listCards') {
    return {
      method: 'GET',
      path: `/api/cards?${buildQuery(payload)}`,
    }
  }
  if (action === 'getCard') {
    return {
      method: 'GET',
      path: `/api/cards/${encodeURIComponent(payload.id)}`,
    }
  }
  if (action === 'getPokemonCards') {
    return {
      method: 'GET',
      path: `/api/pokemon/${encodeURIComponent(payload.id)}/cards?${buildQuery(
        payload
      )}`,
    }
  }
  ...
  if (action === 'getPtcgSyncStatus') {
    return {
      method: 'GET',
      path: '/api/ptcg/sync-status',
    }
  }
  if (action === 'getPtcgSyncRuns') {
    return {
      method: 'GET',
      path: `/api/ptcg/sync-runs?${buildQuery(payload)}`,
    }
  }
  if (action === 'getDailyCardQuiz') {
    return {
      method: 'GET',
      path: '/api/card-quiz/daily',
    }
  }
  if (action === 'submitDailyCardQuiz') {
    return {
      method: 'POST',
      path: '/api/card-quiz/daily/answer',
      data: payload,
    }
  }
  ...
  if (action === 'getHotDecks') {
    return {
      method: 'GET',
      path: `/api/decks/hot?${buildQuery(payload)}`,
    }
  }
  if (action === 'getHotDeckDetail') {
    return {
      method: 'GET',
      path: `/api/decks/detail?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketCards') {
    return {
      method: 'GET',
      path: `/api/pocket/cards?${buildQuery(payload)}`,
    }
  }
  if (action === 'getPocketCard') {
    return {
      method: 'GET',
      path: `/api/pocket/cards/${encodeURIComponent(payload.id)}`,
    }
  }
  if (action === 'getPocketMeta') {
    return {
      method: 'GET',
      path: '/api/pocket/meta',
    }
  }
  if (action === 'listPocketExpansions') {
    return {
      method: 'GET',
      path: `/api/pocket/expansions?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketPacks') {
    return {
      method: 'GET',
      path: `/api/pocket/packs?${buildQuery(payload)}`,
    }
  }
  if (action === 'getPocketPullRates') {
    return {
      method: 'GET',
      path: `/api/pocket/pull-rates?${buildQuery(payload)}`,
    }
  }
  if (action === 'getPocketRarities') {
    return {
      method: 'GET',
      path: '/api/pocket/rarities',
    }
  }
  if (action === 'openPocketPack') {
    return {
      method: 'POST',
      path: '/api/pocket/open-pack',
      data: payload,
    }
  }
  if (action === 'listPocketEvents') {
    return {
      method: 'GET',
      path: `/api/pocket/events?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketMissions') {
    return {
      method: 'GET',
      path: `/api/pocket/missions?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketBattles') {
    return {
      method: 'GET',
      path: `/api/pocket/battles?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketShops') {
    return {
      method: 'GET',
      path: `/api/pocket/shops?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketWonderPicks') {
    return {
      method: 'GET',
      path: `/api/pocket/wonder-picks?${buildQuery(payload)}`,
    }
  }
  if (action === 'listPocketHotDecks') {
    return {
      method: 'GET',
      path: `/api/pocket/hot-decks?${buildQuery(payload)}`,
    }
  }
  if (action === 'getPocketHotDeck') {
    return {
      method: 'GET',
      path: `/api/pocket/hot-decks/${encodeURIComponent(payload.id)}`,
    }
  }
  return null
}
...
function remoteCall(action, data, fallback) {
  if (!config.useRemoteApi || !config.apiBaseUrl || !hasTaro() || !Taro.request) {
...
function cloudCall(action, data, fallback) {
  if (!config.useCloudApi || !hasTaro() || !Taro.cloud) {
    return Promise.resolve(fallback())
  }
  return Taro.cloud
    .callFunction({
...
function apiCall(action, data, fallback) {
  if (config.useStaticApi) {
    let staticApi
    try {
      staticApi = require('./static-api.js')
    } catch (error) {
      return Promise.resolve(fallback())
    }
...
function listPokemon(filters) {
  return apiCall('listPokemon', filters, () => {
    ...
    const pageSize = Math.max(
      1,
      Number(options.pageSize || options.limit || 40)
    )
...
function listLocalCards(filters) {
  const options = filters || {}
  const keyword = String(options.keyword || options.q || '')
    .trim()
    .toLowerCase()
  ...
  const setId = String(options.setId || '')
    .trim()
    .toLowerCase()
  const supertype = String(options.supertype || '')
    .trim()
    .toLowerCase()
  const type = String(options.type || '').trim()
  const rarity = String(options.rarity || '')
    .trim()
    .toLowerCase()
  ...
  const filtered = ptcgSample.CARDS.filter((card) => {
    ...
    if (supertype && String(card.supertype || '').toLowerCase() !== supertype)
      return false
    if (type && !(card.types || []).includes(type)) return false
    if (rarity && String(card.rarity || '').toLowerCase() !== rarity)
      return false
    if (
      pokemonId &&
      !(card.national_pokedex_numbers || []).map(Number).includes(pokemonId)
    )
      return false
    return true
  })
  const sorted = filtered
    .slice()
    .sort((a, b) =>
      String(b.set_release_date || '').localeCompare(
        String(a.set_release_date || '')
      )
    )
  const page = Math.max(1, Number(options.page || 1))
...
function getCardById(id) {
  return apiCall(
    ...
    () => ({
      item:
        ptcgSample.CARDS.find((card) => String(card.id) === String(id)) || null,
      source: 'local-card-seed',
    })
  )
}
  ...
  return apiCall('getPtcgMeta', {}, () => {
    ...
    return {
      item: {
        ...
        rarities: Array.from(new Set(cards.map((card) => card.rarity))).map(
          (id) => ({
            id,
            name:
              (cards.find((card) => card.rarity === id) || {}).rarity_name ||
              id,
          })
        ),
        sets: ptcgSample.SETS,
        series: Array.from(
          new Set(ptcgSample.SETS.map((set) => set.series))
        ).map((id) => ({
          id,
          name: id,
        })),
...
function getDailyCardQuiz() {
  return apiCall('getDailyCardQuiz', {}, () => {
    ...
    return {
      item: {
        quizId: `card-random-local-${Date.now()}-${Math.floor(
          Math.random() * 10000
        )}`,
        answerId: answer.id,
        image: answer.image,
...
function submitDailyCardQuiz(payload) {
  return apiCall('submitDailyCardQuiz', payload, () => ({
    item: {
      ...
      message:
        String(payload.selectedId) === String(payload.answerId)
          ? '猜对了，今天的卡牌灵感到手。'
          : '差一点，看看卡牌详情再熟悉一下。',
    },
    source: 'local-card-seed',
...
function openCardPack(payload) {
  return apiCall('openCardPack', payload || {}, () => {
    const cards = listLocalCards({
      page: 1,
      pageSize: Number((payload && payload.count) || 10),
    }).items
    return {
      item: {
    ...
    () => ({
      items: [
        ...
        },
        {
          ...
          points: 363,
          share: '8.02%',
          url: 'https://limitlesstcg.com/decks/268',
          image: '',
          images: [],
          source: 'Limitless TCG',
        },
        {
          ...
          points: 278,
          share: '6.14%',
          url: 'https://limitlesstcg.com/decks/283',
          image: '',
          images: [],
          source: 'Limitless TCG',
        },
    ...
    .map((section) =>
      [
        ...
        ...(section.cards || []).map(
          (card) =>
            card.line ||
            `${card.count} ${card.name} ${card.set} ${card.number}`.trim()
        ),
      ].join('\n')
    )
    .join('\n\n')
...
function sampleDeckCardImage(set, number) {
  ...
  const cardNumber = /^\d+$/.test(rawNumber)
    ? rawNumber.padStart(3, '0')
    : rawNumber
  if (!setCode || !cardNumber) return ''
  return normalizeRemoteUrl(
    `/assets/limitless/cards/${encodeURIComponent(
      setCode
    )}/${encodeURIComponent(`${setCode}_${cardNumber}_R_EN_LG.png`)}`
  )
}
...
function getHotDeckDetail(deck) {
  return apiCall('getHotDeckDetail', deck || {}, () =>
    sampleHotDeckDetail(deck)
  )
}
function paginatePocket(items, options) {
  ...
  const pageSize = Math.max(
    1,
    Number((options && (options.pageSize || options.limit)) || 30)
  )
  const offset = (page - 1) * pageSize
  return {
...
function listPocketCards(options) {
  ...
  return apiCall('listPocketCards', filters, () => {
    const keyword = String(filters.q || filters.keyword || '')
      .trim()
      .toLowerCase()
    ...
    const items = pocketSample.CARDS.filter((card) => {
      if (
        keyword &&
        ![card.name_zh, card.name_en, card.id, card.artist]
          .concat(
            (card.collections || []).flatMap((item) => [
              item.expansion_id,
              item.expansion_name_zh,
              item.number,
            ])
          )
          .join(' ')
...
function listPocketExpansions(options) {
  return apiCall('listPocketExpansions', options || {}, () =>
    paginatePocket(pocketSample.EXPANSIONS, options || {})
  )
}
function listPocketPacks(options) {
  return apiCall('listPocketPacks', options || {}, () =>
    paginatePocket(pocketSample.PACKS, options || {})
  )
}
    ...
    const cards = Array.from(
      ...
      (unused, index) =>
        Object.assign(
          {},
          pocketSample.CARDS[
            Math.floor(Math.random() * pocketSample.CARDS.length)
          ],
          {
            slot: index + 1,
          }
...
function listPocketEvents(options) {
  return apiCall('listPocketEvents', options || {}, () =>
    paginatePocket(pocketSample.EVENTS, options || {})
  )
}
function pocketCollection(action, sampleFilter, options) {
  return apiCall(action, options || {}, () =>
    paginatePocket(
      pocketSample.EVENTS.filter((event) => sampleFilter.includes(event.type)),
      options || {}
    )
  )
}
...
function listPocketBattles(options) {
  return pocketCollection(
    'listPocketBattles',
    ['soloBattle', 'pvpEmblemBattle', 'rankedPvpSeason'],
    options
  )
}
...
function listPocketWonderPicks(options) {
  return pocketCollection(
    ...
    ['wonderPickFree', 'wonderPickChansey'],
    options
  )
}
function listPocketHotDecks(options) {
  return apiCall('listPocketHotDecks', options || {}, () =>
    paginatePocket(pocketSample.HOT_DECKS, options || {})
  )
}
