...
function getRecentViews(currentPokemon) {
  ...
  const currentById = new Map(
    currentPokemon.map((pokemon) => [Number(pokemon.id), pokemon])
  )
  let changed = false
...
function saveQuizResult(quizId, result) {
  ...
  history[quizId] = Object.assign({}, result, {
    updatedAt: Date.now(),
  })
...
function togglePocketFavorite(id) {
  ...
  write(
    POCKET_FAVORITES_KEY,
    exists ? values.filter((item) => item !== key) : values.concat(key)
  )
  return !exists
}
